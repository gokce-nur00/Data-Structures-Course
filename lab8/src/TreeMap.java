...
public class TreeMap<K,V> extends AbstractSortedMap<K,V> {
  ...
  public int countLessThan(K k){
    ...
    Iterable<Position<Entry<K, V>>> iterable= this.tree.preorder();
    Iterator<Position<Entry<K, V>>> iterator=iterable.iterator();
    while (iterator.hasNext()){
      Position<Entry<K, V>> temp=iterator.next();
      if (temp.getElement().getKey().equals(k)){
        if (iterator.hasNext()){
          Position<Entry<K, V>> temp2= iterator.next();
          return count(temp2);
        }

      }
      else {
        System.out.println("there is no key as "+k);
        return -1;
      }
  ...
  private int count( Position<Entry<K, V>> k){
    if (this.isExternal(k)){
      return 0;
    }
    else
      return 1+count( this.left(k))+count(this.right(k));

  }

  public K preOrderSuccessor(K k){


    Position<Entry<K,V>> temp= treeSearch(root(), k);
    if (left(temp)!=null)
      return left(temp).getElement().getKey();


    Position<Entry<K, V>> nowParent=parent(temp);
    while (nowParent!=null && right(nowParent).equals(temp)){
      temp=nowParent;
    ...
    return right(nowParent).getElement().getKey();
  }


  /*
  Position<Entry<K, V>> temp= this.tree.root();
    K key=null;
    try {
      while (true){
        if (compare(temp.getElement().getKey(),k)==0){//esitse
          if (left(temp)!=null){
            key= this.tree.right(temp).getElement().getKey();
            break;
          }
          if (right(temp)!=null) {
            key = this.tree.right(temp).getElement().getKey();
        ...
        if (compare(temp.getElement().getKey(),k)!=0){//esit degilse
          if (compare(temp.getElement().getKey(),k)<=0)//k>temp
          {
            temp=this.tree.right(temp);
          }
          else if (compare(temp.getElement().getKey(),k)>=0){
            temp=this.tree.left(temp);
          }
      ...
      System.out.println("there is no key as you search.");
    }
