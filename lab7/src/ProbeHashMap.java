...
public class ProbeHashMap<K,V> extends AbstractHashMap<K,V> {
  ...
  public int ortProbe(int ort,K key,int h,int oldTime){
    //ort=onceki ortalama
    //key=bakacagimiz key, h=hash value
    //oldtime= ne kadar once degerden soz ediyoruz
    int newProbe=findSlot(h,key);
    return (ort*oldTime+newProbe)/(oldTime+1);
  }

  public int maxProbe(int max,K key,int h){
    int newProbe=findSlot(h,key);
    if (newProbe>max)
      return newProbe;
    return max;
