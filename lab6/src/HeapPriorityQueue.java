...
public class HeapPriorityQueue<K,V> extends AbstractPriorityQueue<K,V> {
  ...
  public K setKey(int p, K newKey){
    Entry<K,V> old=heap.get(p);
    if (p>=heap.size()|| old.getKey()==null)
      return null;
    K oldKey=old.getKey();
    PQEntry<K,V> newEntry=new PQEntry<>(newKey, old.getValue());
    if (compare(old,newEntry)>0){
      heap.set(p,newEntry);
      upheap(p);
    }
    if (compare(old,newEntry)<=0){
  ...
  public K removeEntry(int p){
    if (heap.get(p)==null)
      return null;
    Entry<K,V> old=heap.get(p);
    Entry<K,V> last=heap.get(heap.size()-1);
    if (compare(old,last)<0){
      heap.remove(last);
      heap.set(p,last);
      downheap(p);
    }
