...
public class DoublyLinkedList<E extends Comparable<E>>{
  ...
  public E remove(int i){
    //use getNode and removeBefore
    Node<E> returnNode=getNode(i);
  ...
  public int find(E data){
      ...
      int indexOfdata=-1;
      while (counter<size()) {
          Node<E> temp=getNode(counter);
          int answer=temp.getElement().compareTo(data);
          if (answer==0){
            indexOfdata=counter;
            break;
          }
  ...
  //private update method from part1
  //adds newData immediately before the given node.
  private void addBefore(Node<E> node, E newData){
    if (isEmpty())
      addBetween(newData,header,trailer);
    if(node.getPrev().equals(header))
      addBetween(newData,header,node);
    else
  ...
  /*private Node<E> getNode(int i){
    ...
    if(i<size/2){
      ...
      for(int search=size-1;search>i;search--){
        walk=walk.getPrev();
      }
    }
    return walk;
  ...
  private Node<E> getNode(int i){
    Node<E> returnType=null;
    if (i<=size()/2)//You must start iterating from head if i is closer to the start
    {
      Node<E> before=header;
      Node<E> after=header.getNext();//that is i=0
      int counter=0;
      while (counter<=i){
        before=after;
        after=after.getNext();
    ...
    {
      Node<E> after=trailer;
      Node<E> before=after.getPrev();//that is the last element of list
      int counter=size()-1;
      while (counter>=i){
        after=before;
