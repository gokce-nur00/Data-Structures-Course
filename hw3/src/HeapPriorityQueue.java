...
public class HeapPriorityQueue<K,V> extends AbstractPriorityQueue<K,V> {
  ...
  protected void downheap(int j) {
    while (hasLeft(j)) {               // continue to bottom (or break statement)
      ...
      int biggestChildIndex = leftIndex;     // although right may be bigger
      if (hasRight(j)) {
          int rightIndex = right(j);
          if (compare(heap.get(leftIndex), heap.get(rightIndex)) < 0)
            biggestChildIndex = rightIndex;  // right child is bigger
      }
      if (compare(heap.get(biggestChildIndex), heap.get(j)) <= 0)
        break;                             // heap property has been restored
      swap(j, biggestChildIndex);
      j = biggestChildIndex;                 // continue at position of the child
    }
  }
